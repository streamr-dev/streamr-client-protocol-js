...
describe('BroadcastMessageSerializerV0', () => {
    describe('deserialize', () => {
        it('correctly parses messages', () => {
            ...
            const result = new BroadcastMessageSerializerV0(streamMsg)
    ...
    describe('serialize', () => {
        describe('serialize version 0', () => {
            ...
            beforeEach(() => {
                const streamMessageArray = [30, ['streamId', 0, 1529549961116, 0, 'address', 'msg-chain-id'], [1529549961000, 0],
                    StreamMessage.CONTENT_TYPES.MESSAGE, '{"valid": "json"}', StreamMessage.SIGNATURE_TYPES.ETH, 'signature']
                broadcastMessage = new BroadcastMessageSerializerV0(StreamMessageFactory.deserialize(streamMessageArray))
            })
            afterEach(() => {
        ...
        it('correctly serializes to version 1', () => {
            ...
            const serialized = new BroadcastMessageSerializerV0(StreamMessageFactory.deserialize(streamMessageArray)).serialize(1)
            assert(typeof serialized === 'string')
            assert.deepEqual(arr, JSON.parse(serialized))
        })
        it('correctly serializes to version 1 with non-default payload', () => {
            const streamMessageArray = [30, ['streamId', 0, 1529549961116, 0, 'address', 'msg-chain-id'], [1529549961000, 0],
                StreamMessage.CONTENT_TYPES.MESSAGE, '{"valid": "json"}', StreamMessage.SIGNATURE_TYPES.ETH, 'signature']
            const serialized = new BroadcastMessageSerializerV0(StreamMessageFactory.deserialize(streamMessageArray)).serialize(1, 29)
            assert(typeof serialized === 'string')
